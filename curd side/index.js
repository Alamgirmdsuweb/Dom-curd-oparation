...
CreateTableData = document.createElement("tr");
...
Heading5 = document.createElement("th");
...
const buildTable = (data) => {
    ...
    data.forEach((users, i) => {
        ...
        let editd = document.createElement("button");
        
        editd.classList = "EditBtn";
        editd.id = "editbtn";
        editd.innerText = "Edit";
        editd.onclick = () => editbtn(users.id);

        let delt = document.createElement("button");
        delt.classList = "DeleteBtn";
        delt.id = "deletbtn";
        delt.innerText = "Delete";
        delt.onclick = () => Deletbtn(users.id);

        for (const key in main) {
            const colum = document.createElement("td");

            colum.innerText = main[key];
            row.append(colum);
    ...
    fetch(url)
        ...
        .then((objectData) => {
            ...
            if (dataForTable.length) {
                ...
                iName1.value = dataForTable[0].name;

                iEmail1.value = dataForTable[0].email;
